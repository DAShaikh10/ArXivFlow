...
- Run pre-annotation quality checks on the scraped dataset before labeling so missing fields, empty reference lists, and suspicious records are surfaced early. _(For the QC workflow, a Streamlit dashboard is available to inspect distributions and flagged records interactively)_
...
│ │ └── quality/
│ │   ├── `__init__.py`        # package initializer for quality checks
│ │   ├── `config.py`          # pre-annotation QC configuration values
│ │   ├── `pre_main.py`        # interactive QC dashboard entrypoint
│ │   └── `pre.py`             # pre-annotation QC checks logic
